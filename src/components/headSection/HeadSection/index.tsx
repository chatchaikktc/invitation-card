...
function HeadSection({ selectedCard }: HeadSectionProps) {
    ...
    return (
        <section className='hero-banner tw-w-full tw-relative tw-bg-cover tw-bg-bottom tw-h-[280px]' style={{ backgroundImage: `url(${card.headSectionImage})` }} id="HeadSection">
            <BreadcrumbSection cardName={card.cardName} />
            <Container>
                <div className='tw-grid tw-grid-cols-2 tw-gap-10 tw-pt-5 tw-px-20 tw-items-center'>
                    <h1 className='tw-text-end tw-text-[35px] tw-font-bold'>{card.cardName}</h1>
