    ...
    return (
        <>
            <div className={twMerge(privilege.positionImage === 'left' ? 'tw-order-1' : 'tw-order-2')}>
                <img src={privilege.imgaePath} alt={privilege.privilegeName} />
            </div>
            <div className={twMerge(privilege.positionImage === 'left' ? 'tw-order-2' : 'tw-order-1')}>
                <h2 className="tw-text-[35px] tw-font-bold tw-mb-3">{tc(privilege.title)}</h2>
                <div className="tw-text-[18px]"><DetailsItems item={tc(privilege.description)} /></div>
                <button className="tw-mt-5 tw-py-[8px] tw-px-4 tw-border-[1px] tw-border-solid tw-border-white tw-text-white hover:tw-text-black hover:tw-bg-white tw-rounded-[100px]">{tc(privilege.buttonText)}</button>
            </div>
        </>
    );
}
